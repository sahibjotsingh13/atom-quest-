...
export default function AnalyticsPage() {
  ...
  const statCards = [
    ...
    { label: "Sheets Submitted", value: overview?.sheetsSubmitted ?? 0, icon: <Target className="w-5 h-5" />, color: "#f59e0b" },
    { label: "Sheets Approved", value: overview?.sheetsApproved ?? 0, icon: <CheckCircle className="w-5 h-5" />, color: "#22c55e" },
    { label: "Submission Rate", value: `${overview?.submissionRate ?? 0}%`, icon: <TrendingUp className="w-5 h-5" />, color: "#a78bfa" },
  ];
    ...
    <AppLayout>
      <div style={{ display: "flex", flexDirection: "column", gap: "2rem", maxWidth: "80rem", margin: "0 auto" }}>
        ...
        <div className="hero-banner">
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <p style={{ fontSize: "0.875rem", color: "rgba(237,232,228,0.5)" }}>
              {cycle ? `${cycle.name} · FY ${cycle.fiscalYear}` : "Organisation-wide performance metrics"}
            </p>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(13rem, 1fr))", gap: "1.25rem" }}>
          {statCards.map((card) => (
            <div key={card.label} className="glass iso-card" style={{ padding: "1.5rem", borderRadius: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                <div style={{ width: "2.5rem", height: "2.5rem", borderRadius: "0.75rem", background: `${card.color}22`, display: "flex", alignItems: "center", justifyContent: "center", color: card.color }}>
                  {card.icon}
                </div>
                <span style={{ fontSize: "0.8125rem", color: "rgba(237,232,228,0.5)", fontWeight: 500 }}>{card.label}</span>
              </div>
              <p style={{ fontSize: "2rem", fontWeight: 800, color: "#ffffff", lineHeight: 1 }}>{card.value}</p>
            </div>
          ))}
        </div>
              ...
              {(checkIns?.byQuarter ?? []).map((q: any) => (
                <div key={q.quarter} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.5rem 0.75rem", borderRadius: "0.625rem", background: "rgba(255,255,255,0.03)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
                    ...
                    <span style={{ fontSize: "0.8125rem", color: "rgba(237,232,228,0.65)" }}>Check-ins</span>
                  </div>
                  <span style={{ fontWeight: 700, fontSize: "0.875rem", color: "#ede8e4" }}>{q.count}</span>
                </div>
              ))}
              {(!checkIns?.byQuarter || checkIns.byQuarter.length === 0) && (
                <p style={{ fontSize: "0.875rem", color: "rgba(237,232,228,0.3)", textAlign: "center", padding: "1rem 0" }}>No check-ins yet</p>
              )}
            </div>
          </div>
          ...
          <div className="glass" style={{ borderRadius: "1rem", padding: "1.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.625rem" }}>
              {(goals?.byStatus ?? []).map((g: any) => {
                const colors = statusColorMap[g.status] ?? statusColorMap.not_started;
                return (
                  <div key={g.status} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.5rem 0.75rem", borderRadius: "0.625rem", background: "rgba(255,255,255,0.03)" }}>
                    <span style={{ fontSize: "0.75rem", fontWeight: 700, padding: "0.25rem 0.625rem", borderRadius: "9999px", background: colors.bg, color: colors.text, textTransform: "capitalize" }}>
                      {g.status.replace(/_/g, " ")}
                    </span>
                    <span style={{ fontWeight: 700, fontSize: "0.875rem", color: "#ede8e4" }}>{g.count} goals</span>
                  </div>
                );
              })}
              {(!goals?.byStatus || goals.byStatus.length === 0) && (
                <p style={{ fontSize: "0.875rem", color: "rgba(237,232,228,0.3)", textAlign: "center", padding: "1rem 0" }}>No goals yet</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="glass" style={{ borderRadius: "1rem", padding: "1.5rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.625rem", marginBottom: "1.25rem" }}>
            <AlertTriangle className="w-4 h-4 text-[#30b0d0]" />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            ...
            {(!thrustAreas || thrustAreas.length === 0) && (
              <p style={{ fontSize: "0.875rem", color: "rgba(237,232,228,0.3)", textAlign: "center", padding: "1rem 0" }}>No thrust area data available</p>
            )}
          </div>
        </div>
      </div>
